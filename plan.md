...
- [x] Replace clear-and-repopulate full sync with staged or diffed sync.
...
- [x] Stop materializing full category result sets for browsing.
...
- [x] Make EPG loading lazy by default.
...
- [x] Lazy-load subtitles/text tracks.
...
- [x] Stop the duplicate sync trigger and per-emission WorkManager re-scheduling.
...
- [x] Stop writing DataStore on every keystroke for EPG hours and parental PIN.
...
- [x] Add composite Room indexes for hot queries.
...
- [x] Replace leading-wildcard `LIKE` search with an indexed search path.
...
- [x] Reduce player overlay progress recomposition frequency.
...
- [x] Avoid resolving playback twice for internal player launches.
...
- [x] Drop the hidden-category `NOT IN` subquery from observed and search queries.
...
- [x] Run sync network calls in parallel where independent.
...
- [x] Stop wiring the OkHttp logging interceptor in release builds.
...
- [x] Gate `PlayerActivity` settings collection on lifecycle state.
...
- [x] Hoist per-recompose / per-call allocations (Brushes, Formatters, Regexes, factories).
  - Current hotspots:
...
- [x] Memoize `HomeScreen` mapped strip lists.
...
- [x] Debounce / cache the live guide Flow on focus changes.
...
- [x] Stop auto-scrolling `LiveCategoryRail` during sync emissions.
...
- [x] Add in-flight de-dupe for `ensureGuide()` and `ensureSeriesLoaded()` so rapid focus/dialog changes cannot trigger duplicate provider calls.
- [x] Configure a shared Coil `ImageLoader` strategy with explicit memory/disk cache sizing and thumbnail-sized requests for rails, grids, logos, and dialog art.
- [x] Add Macrobenchmark coverage for cold start, home load, live category navigation, movie/series grid browsing, search, player launch, and channel zapping.
- [x] Add Baseline Profile generation for the main TV navigation and player startup paths.
- [x] Add provider-sized test fixtures or seeded debug data so paging, search, and sync changes can be measured against realistic catalog volume.

- [x] Eliminate `delay(50)` focus shims across screens.
...
- [x] Memoize player track-option recompute on `onTracksChanged`.
...
- [x] Replace empty ProGuard/R8 rule file with explicit project rules and verify shrinking.
...
- [x] Configure a real release signing config (or document the debug-signed release as intentional).
...
- [x] Add channel up/down and media FF/REW remote key handling.
...
- [x] Tie `MediaItem.LiveConfiguration.targetOffsetMs` to `BufferProfile`.
...
- [x] Implement a real `MediaSessionService` (or drop the `FOREGROUND_SERVICE_MEDIA_PLAYBACK` permission).
...
- [x] Add a cold-start splash composition before the Home screen renders.
...
- [x] Add Room migrations and remove `fallbackToDestructiveMigration` outside debug.
...
- [x] Add `@RewriteQueriesToDropUnusedColumns` and project narrower list-item entities for grids.
...
- [x] Negative-cache `fetchShortEpg` for channels with no provider EPG mapping.
...
- [x] Make the EPG lower bound user-configurable (or drop the hardcoded 4 h look-back).
...
- [x] Remove `room.expandProjection = true` if not strictly needed.
...
- [x] Add Compose stability/recomposition reporting to CI.
...
- [x] Remove dead code in `StreamingTopNav` (zero-sized placeholder Box).
